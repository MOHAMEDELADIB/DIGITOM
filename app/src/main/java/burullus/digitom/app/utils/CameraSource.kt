...
import burullus.digitom.app.utils.CameraSource.Builder
...
private constructor() {
    ...
    fun release() {
        synchronized(cameraLock) {
            stop()
            (frameProcessor ?: return@synchronized).release()
        }
    }
    ...
    fun start(): CameraSource {
        synchronized(cameraLock) {
            ...
            val dummySurfaceTexture =
                SurfaceTexture(DUMMY_TEXTURE_NAME)
            (camera ?: return@synchronized).setPreviewTexture(dummySurfaceTexture)
            (camera ?: return@synchronized).startPreview()
            processingThread = Thread(frameProcessor)
            (frameProcessor ?: return@synchronized).setActive(true)
            (processingThread ?: return@synchronized).start()
        }
        return this
    }
    ...
    fun start(surfaceHolder: SurfaceHolder?): CameraSource {
        synchronized(cameraLock) {
            ...
            camera = createCamera()
            (camera ?: return@synchronized).setPreviewDisplay(surfaceHolder)
            (camera ?: return@synchronized).startPreview()
            processingThread = Thread(frameProcessor)
            (frameProcessor ?: return@synchronized).setActive(true)
            (processingThread ?: return@synchronized).start()
    ...
    fun stop() {
        synchronized(cameraLock) {
            (frameProcessor ?: return@synchronized).setActive(false)
            if (processingThread != null) {
                try {
                    // Wait for the thread to complete to ensure that we can't have multiple threads
                    // executing at the same time (i.e., which would happen if we called start too
                    // quickly after stop).
                    (processingThread ?: return@synchronized).join()
            ...
            if (camera != null) {
                (camera ?: return@synchronized).stopPreview()
                (camera ?: return@synchronized).setPreviewCallbackWithBuffer(null)
                try {
                    ...
                    (camera ?: return@synchronized).setPreviewTexture(null)
                } catch (e: Exception) {
                    Log.e(
                        TAG,
                ...
                (camera ?: return@synchronized).release()
                camera = null
            }
        }
    }
    ...
    fun takePicture(
        ...
        synchronized(cameraLock) {
            if (camera != null) {
                ...
                (camera ?: return@synchronized).takePicture(startCallback, null, null, doneCallback)
            }
        }
    }
    ...
    fun autoFocus(cb: AutoFocusCallback?) {
        synchronized(cameraLock) {
            if (camera != null) {
                ...
                (camera ?: return@synchronized).autoFocus(autoFocusCallback)
            }
        }
    }
    ...
    fun cancelAutoFocus() {
        synchronized(cameraLock) {
            if (camera != null) {
                (camera ?: return@synchronized).cancelAutoFocus()
            }
        }
    }
    ...
    fun setAutoFocusMoveCallback(cb: AutoFocusMoveCallback?): Boolean {
        synchronized(cameraLock) {
            if (camera != null) {
                ...
                (camera ?: return@synchronized).setAutoFocusMoveCallback(autoFocusMoveCallback)
            }
        }
        return true
    }
     ...
     */
    @SuppressLint("InlinedApi", "LogNotTimber", "SyntheticAccessor")
    ...
    private fun setRotation(
        ...
        val windowManager =
            (context ?: return).getSystemService(Context.WINDOW_SERVICE) as WindowManager
        var degrees = 0
        ...
        override fun onPictureTaken(
            ...
            synchronized(cameraLock) {
                if (this@CameraSource.camera != null) {
                    (this@CameraSource.camera ?: return@synchronized).startPreview()
                }
            }
        }
    }
        ...
        fun setNextFrame(data: ByteArray?, camera: Camera) {
            lock.withLock {
                if (mPendingFrameData != null) {
                    camera.addCallbackBuffer((mPendingFrameData ?: return@withLock).array())
            ...
            while (true) {
                lock.withLock {
                    ...
                    outputFrame = Frame.Builder()
                        .setImageData(
                            mPendingFrameData, (previewSize ?: return@withLock).width,
                            (previewSize ?: return@withLock).height, ImageFormat.NV21
                        )
                ...
                // frame.
                try {
                    (mDetector ?: return).receiveFrame(outputFrame)
                } catch (t: Throwable) {
                    Log.e(
                        TAG,
                    ...
                    (camera ?: return).addCallbackBuffer((data ?: return).array())
                }
            }
        }
    ...
    companion object {
        ...
        @SuppressLint("InlinedApi")
        private val CAMERA_FACING_FRONT = CameraInfo.CAMERA_FACING_FRONT
